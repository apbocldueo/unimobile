...
@register_planner("example_planner")
class ExamplePlanner(BasePlanner):
    ...
    def __init__(
        ...
        if preset not in self.PRESETS:
             self.config = {}
             if not prompt_file:
                 raise ValueError(f"ExamplePlanner: no setting '{preset}' and do not provide prompt_file")
        else:
        ...
        logger.info(f"🧩 ExamplePlanner 就绪: Preset={preset}, Prompt={self.target_prompt_file}")
