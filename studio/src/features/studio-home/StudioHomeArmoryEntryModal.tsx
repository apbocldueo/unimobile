...
const templateCardStyle =
  "border-[color:var(--zx-border-light)] bg-[var(--zx-card)] hover:border-[color:var(--zx-primary-border)] hover:bg-[var(--zx-primary-soft)] focus-visible:border-[color:var(--zx-primary-border)] active:scale-[0.99] disabled:pointer-events-none disabled:opacity-50";

type Props = {
  onClose: () => void;
};

type TemplateRow = StudioFlowTemplateListItemDTO & { source: "remote" };

/**
 * Agent 构建入口弹窗：
 * - 有 Studio API 时：预设列表与文档由后端 ``/studio/flow-templates`` 与 ``manifest.yaml`` 驱动；
 * - 无 API 或列表失败时：只保留诚实的空白入口，不构造旧线性 Agent。
 */
export function StudioHomeArmoryEntryModal({ onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 font-sans" role="presentation">
      <button type="button" className="absolute inset-0 bg-black/35 backdrop-blur-[2px]" aria-label="关闭" onClick={onClose} />
      <div
      ...
      >
        <div className="shrink-0 border-b border-[color:var(--zx-divider-ui)] px-5 py-4">
          <h2 id="zx-armory-entry-title" className="text-[16px] font-semibold tracking-tight text-[color:var(--zx-text-title)]">
            {MODAL_TITLE}
          </h2>
          ...
          <section aria-labelledby="zx-armory-templates-heading">
            ...
            {remoteError ? (
              <p className="mt-2 text-[12px] leading-relaxed text-[color:var(--zx-text-muted)] opacity-90">
                无法加载后端模板列表（{remoteError}）。为避免创建旧线性 Agent，当前只提供空白流程。
              </p>
            ) : null}
            {remoteLoading ? (
            ...
            <div className="mt-3 flex flex-col gap-2">
              {templateRows.map((row) => (
                <button
                  ...
                  onClick={() => void onPickRemote(row.id)}
                >
                  <span className="flex items-start gap-3">
                    <span
                      className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-[var(--zx-primary-soft)] text-[color:var(--zx-primary)]"
                      aria-hidden
                    >
                      <IconLayers size={18} strokeWidth={1.85} className="text-current" />
                    </span>
                    <span className="min-w-0 flex-1">
                      <span className="block text-[14px] font-semibold leading-snug text-[color:var(--zx-text-title)]">
                        {row.name}
                        {pickBusyId === row.id ? (
          ...
          <section className="mt-8" aria-labelledby="zx-armory-custom-heading">
            ...
            >
              <span className="flex items-start gap-3">
                ...
                <span className="min-w-0 flex-1">
                  <span className="block text-[14px] font-semibold leading-snug text-[color:var(--zx-text-title)]">空白流程</span>
