...
function AuxShell({ children }: { children: ReactNode }) {
  return (
    <div className="studio-aux-hit flex items-center gap-3 px-3 py-3 text-[14px] font-medium text-[color:var(--zx-text-muted)] hover:text-[color:var(--zx-text-body)]">
      {children}
    </div>
  );
}
    ...
    >
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-6 px-3 pb-4 pt-1">
        <section aria-label="核心入口" className="flex flex-col gap-2">
          ...
          >
            {({ isActive }) => (
              <div
                className={[
                  "group relative flex items-center gap-3 rounded-xl px-3 py-3.5 transition-[background-color,box-shadow,color,transform]",
                  isActive
                    ? "shadow-[0_12px_32px_rgba(90,174,253,0.38)]"
                  : "hover:bg-[var(--zx-nav-hover-bg)]",
                ].join(" ")}
                style={
                  isActive
                    ? {
              ...
              >
                ...
                <span
                  ...
                  className={[
                    ...
                    isActive ? "bg-white/15" : "bg-[var(--zx-nav-icon-bg)] group-hover:bg-[var(--zx-primary-soft)]",
                  ].join(" ")}
                >
                  <IconBot
                    size={ICON}
                ...
                <span className="min-w-0 flex-1 text-[15px] font-semibold leading-tight tracking-tight">
                  流程编辑
                </span>
              </div>
            )}
        ...
        <section className="flex flex-col gap-2" aria-label="辅助操作">
          ...
          >
            <div
              className="flex items-center gap-3 px-3 py-3 text-[14px] font-medium"
              style={{
                backgroundColor: "var(--zx-disabled-bg)",
                color: "var(--zx-text-muted)",
              }}
            >
              <span aria-hidden className="flex h-9 w-9 shrink-0 items-center justify-center rounded-none bg-[var(--zx-nav-icon-bg)]">
                <IconSave size={ICON} strokeWidth={STROKE} style={{ color: "var(--zx-text-muted)" }} />
              </span>
